...
- **Thinking Budget presets** — One-click Thinking Budget buttons (Off / 256 / 512 / 1K / 2K / 4K / Custom) under the context window. The value is now wired through `LLMConfig` end-to-end (prefs sync, `set_config`/`get_config`) and sent to llama-server on the local path only — never to remote APIs. Fixes the knob existing without ever reaching the request body.
